...
class StudentAgent(Agent):
    """
    The Architect (Champion Version)
    --------------------------------
    ...
    def __init__(self):
        ...
        # Pre-calculating direction vectors for instant lookup
        self.directions = [
            (-1, 0),
            (1, 0),
            (0, -1),
    ...
    def step(self, chess_board, my_pos, adv_pos):
        ...
        time_limit = 1.90  # Safety buffer (Max is usually 2.0s)
        ...
        # Use internal FAST move generation (Critical for hitting Depth 4+)
        ...
        try:
            while True:
                # Check time BEFORE diving into next depth
                if time.time() - start_time > time_limit:
                    break

                # Standard Stable Alpha-Beta Search
    ...
    def alphabeta(
        ...
        # Time Check: Only check every 1024 nodes to save CPU cycles
        ...
        for m in valid_moves:
            ...
            if tt_move and m == tt_move:
                ...
                # Heuristic: Prioritize Captures, then Clones
                r_dest, c_dest = m.get_dest()
                r_src, c_src = m.get_src()

                # Fast neighbor check for captures
                is_capture = False
                for dr, dc in [
                    (-1, 0),
                    (1, 0),
    ...
    def fast_evaluate(self, board, player, opponent):
        ...
        Optimized weights derived from 200+ generations of self-play training.
        """
        ...
        # Vectorized Quad Check (2x2 Structures)
        b_tl = board[:-1, :-1]
        b_tr = board[:-1, 1:]
        b_bl = board[1:, :-1]
        b_br = board[1:, 1:]
        ...
        # Mobility Check (Expensive, so we gate it)
        score_mobility = 0
        if w_mob > 5.0:
        ...
        # Positional Map (Edges, Corners, Center)
