...
class MoveCoordinates:
    """
    MoveCoordinates is a simple helper to store (row, column) tuples.
    Required by the game engine for the final return value.
    """
...
class StudentAgent(Agent):
    ...
    Monolith
    --------------------------------
    Strategy: Genetic Algorithm Optimized Weights
    Search: Iterative Deepening Alpha-Beta with Transposition Tables
    Heuristics: Killer Moves, History Heuristic
    Speed: Custom Tuple-based internal move execution
    Compatibility: Wrapper class for game engine compliance
    """
    ...
    def __init__(self):
        ...
        self.history = np.zeros((7, 7), dtype=int)  # History Heuristic (7x7 matrix)
    ...
    def step(self, full_board, my_pos, adv_pos):
        ...
        time_limit = 1.88  # Safety buffer (Max 2.0s)
        ...
        player = my_pos
        opponent = adv_pos
        valid_moves = self.get_valid_moves_fast(full_board, player)
        if not valid_moves:
            return None
    ...
    def get_valid_moves_fast(self, board, player):
        ...
        Returns: List of Tuples [ ((r_src, c_src), (r_dest, c_dest)), ... ]
        """
        moves = []
    ...
    def apply_move_local(self, board, move, player):
        """
        Internal fast move execution.
        Bypasses helpers.py entirely to handle Tuples and run faster.
        SAFE: Correctly ignores Walls (3) and only flips Opponents.
        """
        (r_src, c_src), (r_dest, c_dest) = move
        board[r_dest, c_dest] = player
        if abs(r_dest - r_src) > 1 or abs(c_dest - c_src) > 1:
            board[r_src, c_src] = 0  # Remove source if it's a jump
